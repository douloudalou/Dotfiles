...
class dcapSpider(scrapy.Spider):
    ...
    def attachment(self, response):
        ...
        import requests
        zipFileName = f"{response.meta['index']}.zip"
        zipPath = Path(f"~/Development/dcapSpider/spiders/pdfFiles/{zipFileName}")
        extractPath = Path(r"~/Development/dcapSpider/spiders/extractFiles")
        ...
        files = {
            'nric_front': open(file, 'rb'),
            'nric_back': open(file, 'rb'),
            'income_doc': open(file, 'rb')
        }
